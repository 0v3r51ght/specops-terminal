#!/usr/bin/env bash
# S.O.T · SpecOps Terminal v2.7 — FULL AUDIT RELEASE
# Made by 0v3r51ght
...
execute_shell_command() {
    ...
    # Every mapped action in Lab mode shows the exact rendered command before
    # any y/N, RUN or CHANGE confirmation. The custom terminal is separate.
    preview_lab_command "$audit"
...
preview_monitor_on_command() {
    local iface=$1 helper quoted_iface
    [[ "$ACTIVE_MODE" == lab ]] || return 0
    helper=$(root_command_prefix) || return 1
    printf -v quoted_iface '%q' "$iface"
    preview_lab_command "${helper:+$helper }airmon-ng check kill && ${helper:+$helper }airmon-ng start $quoted_iface"
}

preview_monitor_off_commands() {
    local mon=$1 managed=$2 helper quoted_mon quoted_managed plan
    [[ "$ACTIVE_MODE" == lab ]] || return 0
    helper=$(root_command_prefix) || return 1
    printf -v quoted_mon '%q' "$mon"
    printf -v quoted_managed '%q' "$managed"
    plan="${helper:+$helper }airmon-ng stop $quoted_mon"$'\n'
    plan+="${helper:+$helper }iw dev $quoted_managed set type managed    # only if required"$'\n'
    plan+="${helper:+$helper }ip link set dev $quoted_managed up"$'\n'
    plan+="# restart the network services recorded before monitor mode"$'\n'
    plan+="${helper:+$helper }rfkill unblock wifi                       # when rfkill is installed"$'\n'
    if [[ -n "$helper" ]]; then
        plan+="nmcli radio wifi on || $helper nmcli radio wifi on          # when nmcli is installed"
    else
        plan+="nmcli radio wifi on                                      # when nmcli is installed"
    fi
    preview_lab_command "$plan"
}
...
monitor_on() {
    ...
    preview_monitor_on_command "$iface" || {
        error "Could not build the monitor-mode command preview."
        return 1
    }
...
monitor_off() {
    ...
    preview_monitor_off_commands "$mon" "$managed" || {
        error "Could not build the networking-restoration command preview."
        return 1
    }
    ...
    if (( rc == 0 )); then
        rm -f -- "$(workspace_path)/monitor-interface.state" \
            "$(workspace_path)/managed-interface.state" \
            "$(workspace_path)/monitor-services.state"
    else
    ...
    while true; do
        ...
        case "$choice" in
            ...
            2)
                ...
                local line_count
                line_count=$(awk 'END { print NR }' "$(scope_file)")
                [[ "$number" =~ ^[0-9]+$ ]] && (( number >= 1 && number <= line_count )) || {
                    error "Choose an existing scope line between 1 and ${line_count:-0}."
                    pause
                    continue
                }
...
professional_terminal() {
    local rc=0 run_dir rcfile quoted_home
    run_dir=$(workspace_path) || {
        error "Could not resolve the active workspace."
        return 1
    }
    rcfile=$(private_temp_file "$(workspace_path)/tmp" 'sot-custom-terminal' '.bash') || {
        error "Could not create the custom terminal startup file."
        return 1
    }
    printf -v quoted_home '%q' "$HOME/.bashrc"
    cat > "$rcfile" <<EOF
export SOT_CUSTOM_TERMINAL=1
export SOT_WORKSPACE=$(printf '%q' "$run_dir")
if [[ -r $quoted_home ]]; then
    source $quoted_home
fi
...
HISTSIZE=5000
if [[ -t 1 && -z "\${NO_COLOR:-}" ]]; then
    PS1='\[\033[40m\033[38;2;0;255;65m\]\[\033[1m\]S.O.T\[\033[0m\033[40m\033[38;2;0;145;0m\][${ACTIVE_WORKSPACE}]\[\033[38;2;221;255;221m\] \u@\h \[\033[38;2;0;210;25m\]\w\[\033[0m\]\n\[\033[40m\033[38;2;0;255;65m\]❯ \[\033[0m\]'
    PS2='\[\033[40m\033[38;2;0;145;0m\]… \[\033[0m\]'
    printf '\033]0;S.O.T Custom Professional Terminal · ${ACTIVE_WORKSPACE}\007'
else
    PS1='S.O.T[${ACTIVE_WORKSPACE}] \u@\h \w\n> '
    PS2='... '
fi
export PS1 PS2 HISTCONTROL HISTSIZE
EOF
    chmod 600 "$rcfile" || {
        secure_remove "$rcfile" || true
        error "Could not secure the custom terminal startup file."
        return 1
    }

    header
    printf '%b\n\n' "${WHITE}${BOLD}S.O.T CUSTOM PROFESSIONAL TERMINAL${RESET}"
    printf '%b\n' "${GREEN}Workspace:${RESET} ${WHITE}$run_dir${RESET}"
    printf '%b\n' "${GREEN}Shell:${RESET} ${WHITE}unrestricted interactive Bash${RESET}"
    printf '%b\n\n' "${GREY}Use any normal command or installed tool. Type 'exit' or press Ctrl-D to return to S.O.T.${RESET}"
    (
        cd -- "$run_dir" || exit 1
        exec bash --noprofile --rcfile "$rcfile" -i <&3
    )
    rc=$?
    secure_remove "$rcfile" || true
    printf '%b' "$RESET"
    if (( rc != 0 )); then
        warn "The custom interactive Bash shell exited with status $rc."
    fi
    return 0
}
...
MAIN MENU WORKFLOW
  ...
  5. S.O.T Custom Professional Terminal: open a normal unrestricted interactive Bash shell in the active workspace.
...
RUNNING A TOOL
  ...
  whether its executable or mapped package is present, and every mapped action. Select an action and
  enter the requested values. In Lab mode S.O.T displays the exact rendered command before every y/N,
  RUN, CHANGE, MONITOR or RESTORE confirmation. Once execution starts, the terminal is cleared and shows a
...
S.O.T CUSTOM PROFESSIONAL TERMINAL
  Option 5 opens an unrestricted interactive Bash shell in the active workspace with a black-and-green
  S.O.T prompt and branding. It loads the user's normal Bash configuration, then reapplies the S.O.T
  prompt so aliases, shell functions, interpreters, package managers, sudo and interactive tools remain
  available. S.O.T does not parse, filter, rewrite, confirm or audit commands entered inside this shell.
  Type exit or press Ctrl-D to close it and return to S.O.T.
...
how_to_use() {
    ...
    while true; do
        ...
        printf '8. S.O.T Custom Professional Terminal\n'
...
EOF
               ;;
            8) help_page 'S.O.T CUSTOM PROFESSIONAL TERMINAL' <<'EOF'
The S.O.T Custom Professional Terminal opens an unrestricted interactive Bash shell in the active
workspace. It loads the user's Bash configuration and then reapplies S.O.T's black-and-green prompt,
so normal commands, pipelines, redirections, aliases, shell functions, interpreters and interactive
programs remain available. S.O.T does not parse, filter, block, rewrite, confirm or audit commands entered
inside this shell.
...
entered in the S.O.T Custom Professional Terminal are unrestricted and are the user's responsibility.
...
fork-bomb patterns. The S.O.T Custom Professional Terminal is unrestricted and does not apply mapped-action safeguards.
EOF
               ;;
...
main_menu() {
    ...
    while true; do
        ...
        printf '5. S.O.T Custom Professional Terminal\n'
...
self_check() {
    ...
    [[ "$source_text" == *"SpecOps Terminal v${VERSION}"* && "$source_text" == *"VERSION=\"${VERSION}\""* ]] || {
        error "Displayed version and VERSION variable are inconsistent."; failed=1; }
    ...
    local execute_policy preview_policy
    execute_policy=$(declare -f execute_shell_command)
    preview_policy=$(declare -f preview_lab_command)
    [[ "$execute_policy" == *'tee -a -- "$logfile" | clean_result_stream'* &&
       "$execute_policy" == *'preview_lab_command "$audit"'* &&
       "$execute_policy" == *'if high_risk_command'* &&
       "$execute_policy" == *'system_change_command "$command"'* &&
       "$preview_policy" == *'[[ "$ACTIVE_MODE" == lab ]]'* &&
       "$preview_policy" == *'Command to execute:'* &&
    ...
    [[ "$terminal_policy" == *'exec bash --noprofile --rcfile "$rcfile" -i'* &&
       "$terminal_policy" == *'<&3'* && "$terminal_policy" == *'S.O.T CUSTOM PROFESSIONAL TERMINAL'* &&
       "$terminal_policy" == *'PS1='* && "$terminal_policy" == *'source $quoted_home'* &&
       "$terminal_policy" != *'catastrophic_command'* && "$terminal_policy" != *'high_risk_command'* &&
       "$terminal_policy" != *'system_change_command'* ]] || {
        error "S.O.T Custom Professional Terminal is not a themed unrestricted interactive Bash shell."; failed=1; }
    local monitor_policy
    monitor_policy=$(declare -f monitor_on; declare -f monitor_off; declare -f preview_monitor_on_command; declare -f preview_monitor_off_commands)
    [[ "$monitor_policy" == *'wireless_interface_type'* &&
       "$monitor_policy" == *'monitor_interface_on_phy'* &&
       "$monitor_policy" == *'preview_monitor_on_command "$iface"'* &&
       "$monitor_policy" == *'preview_monitor_off_commands "$mon" "$managed"'* &&
       "$monitor_policy" == *'Managed-mode verification failed'* ]] || {
        error "Monitor-mode preview, verification or restoration checks are incomplete."; failed=1; }
    local result_filter
...
verify_runtime_dependencies() {
    local cmd missing=0
    for cmd in awk base64 cat chmod date find grep mkdir mktemp mv nl rm rmdir sed sleep sort stat tee touch uname; do
