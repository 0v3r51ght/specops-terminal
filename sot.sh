...
register_tool 'Recon & Enumeration' theHarvester theharvester theharvester theHarvester 'Email, host and subdomain discovery.'
register_func theHarvester DuckDuckGo dGhlSGFydmVzdGVyIC1kIHtkb21haW59IC1iIGR1Y2tkdWNrZ28gLWwge2xpbWl0fQ==
register_func theHarvester Bing dGhlSGFydmVzdGVyIC1kIHtkb21haW59IC1iIGJpbmcgLWwge2xpbWl0fQ==
register_func theHarvester DNSDumpster dGhlSGFydmVzdGVyIC1kIHtkb21haW59IC1iIGRuc2R1bXBzdGVyIC1sIHtsaW1pdH0=
register_func theHarvester 'Custom source' dGhlSGFydmVzdGVyIC1kIHtkb21haW59IC1iIHtzb3VyY2V9IC1sIHtsaW1pdH0=
...
write_arch_pacman_config() {
    ...
    ARCH_EMPTY_HOOKS="$SOT_HOME/package-sources/pacman-hooks-empty"
    [[ -r /etc/pacman.d/mirrorlist ]] || { error "Arch mirror list is missing: /etc/pacman.d/mirrorlist"; return 1; }
    safe_mkdir "$SOT_HOME/package-sources" || return 1
    safe_mkdir "$ARCH_EMPTY_HOOKS" || return 1
...
validate_value() {
    ...
    case "$key" in
        wireless_card|monitor_interface|iface) validate_interface "$value" || return 1 ;;
        port|channel|depth|limit|max|min|min_length|rate)
...
monitor_off() {
    local mon managed answer logfile rc=0 step_rc
    command -v airmon-ng >/dev/null 2>&1 || { error "airmon-ng is not installed."; return 1; }
    printf '%b\n' "${WHITE}Current wireless interfaces:${RESET}"
    list_wireless_interfaces | sed 's/^/  - /'
    read -r -u 3 -p "Monitor interface to stop: " mon
    ...
    logfile=$(output_log_path monitor_off) || { error "Could not create an evidence log."; return 1; }
    {
        ...
        for step_rc in 1 2 3; do
            ip link show dev "$managed" >/dev/null 2>&1 && break
            sleep 1
        done
        if ip link show dev "$managed" >/dev/null 2>&1; then
        ...
        sleep 1
        command -v iw >/dev/null 2>&1 && iw dev || true
    ...
    if (( rc != 0 )); then error "One or more restoration steps failed. Review: $logfile"; fi
    return "$rc"
}
...
resolve_console_directory() {
    local requested=$1 base candidate
    base=$(cd -- "$(workspace_path)" && pwd -P) || return 1
    candidate=$(cd -- "$requested" 2>/dev/null && pwd -P) || return 1
    case "$candidate" in
        "$base"|"$base"/*) printf '%s' "$candidate" ;;
        *) return 1 ;;
    esac
}

restart_network_manager_console() {
    if command -v systemctl >/dev/null 2>&1 && systemctl list-unit-files NetworkManager.service --no-legend 2>/dev/null | grep -q NetworkManager; then
        execute_shell_command 'sudo systemctl restart NetworkManager.service' nm_restart 'sudo systemctl restart NetworkManager.service'
    elif command -v service >/dev/null 2>&1; then
        execute_shell_command 'sudo service network-manager restart' nm_restart 'sudo service network-manager restart'
    else
        error "No supported NetworkManager service command was detected."
        return 1
    fi
}

custom_terminal_help() {
    cat <<'EOF'
Mapped console commands:
  help, clear, pwd, cd <path>, ls, tools, status, scope
  install <mapped-tool>, report, json export, note
  ip link, iw dev, lsusb, rfkill, nm restart, wifi on, wifi off
  back / exit

Only the commands listed above are accepted. Arbitrary shell execution is disabled.
EOF
}

custom_terminal() {
    local raw arg candidate
    CWD=$(cd -- "$(workspace_path)" && pwd -P)
    while true; do
        printf '%b' "${DARK}┌─[${NEON}S.O.T${DARK}@${GREEN}${ACTIVE_WORKSPACE}${DARK}]─[${WHITE}${CWD}${DARK}]\n└──>> ${RESET}"
        read -r -u 3 raw || return
        [[ -n "$raw" ]] || continue
        case "$raw" in
            back|exit|quit|q) return ;;
            help|'?') custom_terminal_help ;;
            clear) header; printf '%b\n' "${WHITE}${BOLD}MAPPED COMMAND CONSOLE${RESET}" ;;
            pwd) printf '%s\n' "$CWD" ;;
            ls)
                printf '%s\n' "Directory: $CWD"
                command ls -la -- "$CWD" ;;
            tools)
                for cat in "${CATEGORIES[@]}"; do
                    printf '%s: ' "$cat"
                    while IFS= read -r tool; do [[ -n "$tool" ]] && printf '%s ' "$(tool_label "$tool")"; done <<< "${CATEGORY_TOOLS[$cat]}"
                    printf '\n'
                done ;;
            ...
            note) notes_menu; header ;;
            'ip link') execute_shell_command 'ip link show' wireless_quick 'ip link show' ;;
            'iw dev') execute_shell_command 'iw dev' wireless_quick 'iw dev' ;;
            lsusb) execute_shell_command 'lsusb' wireless_quick 'lsusb' ;;
            rfkill) execute_shell_command 'rfkill list' wireless_quick 'rfkill list' ;;
            'nm restart') restart_network_manager_console ;;
            'wifi on') execute_shell_command 'nmcli radio wifi on' wifi_on 'nmcli radio wifi on' ;;
            'wifi off') execute_shell_command 'nmcli radio wifi off' wifi_off 'nmcli radio wifi off' ;;
            cd|cd\ *)
                arg=${raw#cd}; arg=${arg# }
                [[ -n "$arg" ]] || arg=$(workspace_path)
                [[ "$arg" == /* ]] || arg="$CWD/$arg"
                if candidate=$(resolve_console_directory "$arg"); then
                    CWD=$candidate
                else
                    error "Console navigation must stay inside the active workspace."
                fi ;;
            install\ *)
                arg=${raw#install }
                if tool_exists "$arg"; then install_tool_package "$arg"; else error "Use a mapped tool name."; fi ;;
            *) error "Unknown command. Type 'help' to see the mapped commands." ;;
        esac
    done
}
...
MAIN MENU WORKFLOW
  ...
  5. Mapped Command Console: use only the listed safe console commands; arbitrary shell input is rejected.
...
MAPPED COMMAND CONSOLE
  Type help inside the console to see the accepted commands. The console supports navigation inside
  the current workspace, status, scope display, report generation, JSON export, encrypted notes,
  selected wireless checks and installation of a named mapped tool. It does not execute arbitrary
  shell commands.
...
how_to_use() {
    ...
    while true; do
        ...
        printf '8. Mapped command console\n'
...
URLs, CIDRs and wireless identifiers are validated before execution. Passive commands request y/N.
Active or credential-testing commands require RUN. Network or system-state changes require CHANGE.
...
EOF
               ;;
            8) help_page 'MAPPED COMMAND CONSOLE' <<'EOF'
The console is intentionally not a general shell. Type help to list accepted commands. You can inspect
pwd/ls, move with cd, list mapped tools, display status or scope, install a named mapped tool, generate
reports/JSON, manage notes and run selected wireless-status actions. Unknown input is rejected.

The console starts in the active workspace. Use back, exit, quit or q to return. Package installation
still passes through the same distro and signature policy as the main install menu. System and wireless
commands still use the normal confirmation prompts.
EOF
               ;;
...
scope accurate and review command templates in the mapped tool reference when needed. Catastrophic disk formatting, root deletion, shutdown/reboot and
fork-bomb patterns are blocked. Package removal and arbitrary console execution are disabled.
...
Catastrophic disk formatting, root-filesystem deletion, shutdown/reboot and fork-bomb patterns are
blocked. Arbitrary shell execution and package removal are disabled.
EOF
               ;;
...
main_menu() {
    ...
    while true; do
        ...
        printf '5. Mapped Command Console\n'
        ...
        case "$choice" in
            ...
            3) workspace_menu ;;
            4) scope_menu ;;
            5) header; printf '%b\n' "${WHITE}${BOLD}MAPPED COMMAND CONSOLE${RESET}"; custom_terminal ;;
            6) reports_menu ;;
            7) profiles_menu ;;
            8) status_report ;;
            9) select_distro ;;
...
self_check() {
    ...
    for cat in "${CATEGORIES[@]}"; do
        ...
        while IFS= read -r tool; do
            ...
            for ((i=1; i<=${TOOL_FUNC_COUNT[$tool]:-0}; i++)); do
                ...
                if [[ "$decoded" != __monitor_on__ && "$decoded" != __monitor_off__ ]]; then
                    syntax=$decoded
    ...
    [[ "$execute_policy" == *'tee -a -- "$logfile" | clean_result_stream'* && "$execute_policy" != *'Command preview:'* && "$execute_policy" != *'Command completed.'* ]] || {
        error "Mapped command output is not cleanly separated from audit metadata."; failed=1; }
    local result_filter
