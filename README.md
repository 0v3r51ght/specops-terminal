...
This lets all users launch S.O.T with the `sot` command:

```bash
chmod +x sot.sh
sudo cp sot.sh /usr/local/bin/sot
sudo chmod +x /usr/local/bin/sot
sot --check
sot
```

Do not run the S.O.T application itself with `sudo`.
...
| **Mapped Command Console** | Use a restricted command interface for S.O.T functions; it is not a general shell. |
...
| **theHarvester** | All-source email, host, IP, URL and subdomain discovery with automatic fallbacks. | Full discovery using all sources with fallback; Quick all-source discovery; Deep all-source discovery; Full all-source discovery with DNS resolution |
...
### theHarvester fallback behaviour

Every theHarvester action starts with the installed tool's native all-source mode:

```bash
theHarvester -d example.com -l 500 -b all
```

If the all-source pass fails, reports a fatal error or returns no recognised result sections, S.O.T reads the source list exposed by the installed theHarvester version and retries supported public/no-key sources individually. Running the fallback sources separately prevents one unavailable source from stopping the remaining sources.

The four mapped modes are:

- **Full discovery:** asks for a result limit and runs all sources with automatic fallback.
- **Quick discovery:** uses a result limit of 200.
- **Deep discovery:** uses a result limit of 1000.
- **Full discovery + DNS resolution:** enables DNS resolution and keeps the same automatic fallback process.

API-backed sources are used by the initial all-source pass when the user has configured the required credentials in theHarvester. S.O.T does not collect or store those API keys.
...
**S.O.T · SpecOps Terminal v2.4 — Made by 0v3r51ght**
