...
### Option 1 — Run `sot.sh` from its current folder

Open a terminal in the folder containing `sot.sh`, make it executable, run the built-in check and then launch it:
...
You only need to run `chmod +x sot.sh` once unless the executable permission is removed later.

### Option 2 — Install the `sot` command system-wide

This lets the user launch S.O.T by entering `sot` from any directory:
...
The `sudo` commands above install only the launcher file. **Do not launch the S.O.T application itself with `sudo`.** S.O.T requests elevated permission only for approved individual operations that require it.

### Option 3 — Install the `sot` command for the current user

This option does not write to system directories:
...
Ensure `~/.local/bin` is in the current user's `PATH`:
...
```bash
./sot.sh --version
# or, after installing the launcher:
sot --version
```
...
3. Restore executable permission with `chmod +x sot.sh`, or copy the replacement file back to the installed launcher location.
...
For portable use, remove `sot.sh` from the folder where it was saved:
